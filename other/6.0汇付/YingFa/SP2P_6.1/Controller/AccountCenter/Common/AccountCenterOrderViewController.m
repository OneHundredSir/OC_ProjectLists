...
- (void)initView
{
    ...
    //列表初始化
    _tableView = [[SKSTableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 44) style:UITableViewStylePlain];
...
- (NSArray *)contents
{
    if (!_contents) {
        ...
        _contents = @[
                      @[@[@"借款子账户",@"借款账单",@"审核中",@"招标中",@"还款中", @"已成功",@"资料审核"]],
                      @[@[@"理财子账户",@"理财账单", @"投标记录",@"等待满标",@"收款中",@"已完成",@"债权管理",@"理财统计"]],
                      @[@[@"账户管理",@"账户信息",@"资金记录",@"充值",@"提现",@"账户安全",@"我的信用等级",@"我的收藏",@"我关注的用户",@"黑名单"]]
                      ];
    }
...
-(void)tableView:(UITableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DLOG(@"dkfdkjhdjkfhkjdhkjf is %d",[[NSString stringWithFormat:@"%ld%ld%ld", (long)indexPath.section,(long)0,(long)indexPath.row] intValue]);
    indexPath.subRow = 0;
    
    switch ([[NSString stringWithFormat:@"%ld%ld%ld", (long)indexPath.section,(long)indexPath.subRow,(long)indexPath.row] intValue]) {
    ...
    if (checked ==1) {
        ...
        if ([db open])
        {
            ...
            while ([rs next])
            {
                ...
                DLOG(@"TableItemCount %ld", (long)count);
                listcount = count;
            }
            ...
            if (listcount<8)
            {
                ...
                [db executeUpdate:@"INSERT INTO UserSet (Id,Tag,Checked) VALUES (?,?,?)",IDstr,[NSNumber numberWithInt:checkbox.tag],[NSNumber numberWithInt:checked]];
