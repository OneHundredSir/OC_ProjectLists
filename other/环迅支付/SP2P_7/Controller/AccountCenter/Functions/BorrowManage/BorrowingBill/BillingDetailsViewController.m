...
#import <IPSSDK/RepaymentViewController.h>
#import "HXData.h"
#import "JSONString.h"


@interface BillingDetailsViewController ()<UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate, HTTPClientDelegate,repaymentDelegate>
{
    NSArray *tableArr;
    ...
    NSString * merBillNO;
    NSString * merBillNOStr1;
    NSString * merBillNOStr2;
    NSString * SmDate;
    
    NSMutableArray * pDetailArr;
...
- (void)initData
{
    ...
    pDetailArr = [[NSMutableArray alloc] init];
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    }else if(_requestType == 1) // 还款
    {
        if ([error isEqualToString:@"-1"])
        {
            ...
            item.pRepaymentDate = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pRepaymentDate"]];
            item.pMerBillNo = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pMerBillNo"]];
            item.pRepayType = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pRepayType"]];
            if ([dics objectForKey:@"pIpsAuthNo"]) {
                item.pIpsAuthNo = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pIpsAuthNo"]];
            }else {
                item.pIpsAuthNo = @"";
            }
            item.pOutAcctNo = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pOutAcctNo"]];
            item.pOutAmt = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pOutAmt"]];
            item.pOutFee = [NSString stringWithFormat:@"%.2f",[[dics objectForKey:@"pOutFee"] floatValue]];
            item.pS2SUrl = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pS2SUrl"]];
            
            NSString *json = [dics objectForKey:@"pDetails"];
            pDetailArr = [NSJSONSerialization JSONObjectWithData:[json dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingAllowFragments error:nil];
//            NSLog(@"pDetailArr = %@",pDetailArr);
            

            item.pInAmt = [NSString stringWithFormat:@"%@",[dics objectForKey:@"pOutAmt"]];
            ...
            merBillNO = [NSString stringWithFormat:@"MER%@", [dateformatter stringFromDate:date]];
            
            payeeI = 0;
            NSDate * date2 = [NSDate date];
            NSDateFormatter * dateformatter2 = [[NSDateFormatter alloc] init];
            [dateformatter2 setDateFormat:@"YYYYMMdd"];
            SmDate = [dateformatter2 stringFromDate:date2];
            
            
            
            [RepaymentViewController  repaymentWithPlatform:@""
                                                   pMerCode:item.pMerCode
                                                     pBidNo:item.pBidNo
                                             pRepaymentDate:SmDate
                                                 pMerBillNo:merBillNO
                                                 pRepayType:item.pRepayType
                                                 pIpsAuthNo:item.pIpsAuthNo
                                                 pOutAcctNo:item.pOutAcctNo
                                                    pOutAmt:item.pOutAmt
                                                    pOutFee:item.pOutFee
                                                    pS2SUrl:item.pS2SUrl
                                                   pDetails:pDetailArr
                                               pWhichAction:@"3"
                                             ViewController:self
                                                   Delegate:self
...
- (void)repaymentResult:(NSString *)pErrCode ErrMsg:(NSString *)pErrMsg MerCode:(NSString *)pMerCode MerBillNo:(NSString *)pMerBillNo BidNo:(NSString *)pBidNo RepaymentDate:(NSString *)pRepaymentDate IpsBillNo:(NSString *)pIpsBillNo OutAcctNo:(NSString *)pOutAcctNo OutAmt:(NSString *)pOutAmt OutFee:(NSString *)pOutFee OutIpsFee:(NSString *)pOutIpsFee Memo1:(NSString *)pMemo1 Memo2:(NSString *)pMemo2 Memo3:(NSString *)pMemo3
{
    DLOG(@"%@",pErrMsg);
    ...
    switch (buttonIndex) {
        ...
        case 1:
        {
            if (alertView.tag == 60)
            {
                ...
                NSLog(@"确定按钮===%@===", [alertView textFieldAtIndex:0].text);
