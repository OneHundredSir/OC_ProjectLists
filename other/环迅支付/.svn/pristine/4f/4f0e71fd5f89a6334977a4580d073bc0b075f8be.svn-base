...
- (void)initView
{
    ...
    _nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(MSWIDTH * 0.5 - 100, 120, 200, 30)];
    ...
    _otherBtn.frame = CGRectMake(self.view.frame.size.width-120, self.view.frame.size.height-40, 100, 15);
    _otherBtn.titleLabel.font = [UIFont systemFontOfSize:13.0];
    [_otherBtn setTitle:@"登录其他账户" forState:UIControlStateNormal];
...
- (void)otherBtnClick
{
    ...
    VerifyPasswordView.forgetTag = 2;
...
//    [[AppDefaultUtil sharedInstance] setDefaultUserNoPassword:@""]; // 清除缓存密码(登录默认记住密码)
