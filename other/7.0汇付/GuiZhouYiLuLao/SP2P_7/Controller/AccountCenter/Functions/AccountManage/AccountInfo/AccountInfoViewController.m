...
//    _titleArr = @[@"真实姓名",@"性别",@"年龄",@"身份证号码", @"文化程度",@"户口所在地",@"婚姻情况",@"购车情况",@"购房情况"];
    _titleArr = @[@"真实姓名",@"性别",@"年龄",@"身份证号码"];//registerType,0.1判断企业用户 ，
...
- (void)initView
{
     ...
     */
    _accountTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(headerView.frame)+10, self.view.frame.size.width, 450) style:UITableViewStylePlain];
...
//            
//        }else
//            if ([_screenTypeStr isEqualToString:@"年龄"]) {
//            
//            [_detailArr  replaceObjectAtIndex:1 withObject:_ageStr];
//            
//        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if (registerType == 1)
    {
        ...
        if(indexPath.row == 0 && [cell.detailTextLabel.text isEqualToString:@"未设置"]){
            ...
            [cell.contentView addSubview:_nameField];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }else if(indexPath.row == 1 && [cell.detailTextLabel.text isEqualToString:@"未设置"]){
            cell.detailTextLabel.hidden = YES;
            [cell.contentView addSubview:_cardField];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (_requestType == 0) {
            ...
            if ([[dics objectForKey:@"regsterType"] integerValue] == 1) {
                _titleArr = @[@"真实姓名",@"身份证号码"];
                _saveItem.title = @"";
                [_saveItem  setAction:@selector(noAction)];
            }
            registerType = [[dics objectForKey:@"regsterType"] integerValue];
...
-(void) submitAccountInfo
{
    ...
    if (registerType == 0) {
        ...
        [parameters setObject:@(0) forKey:@"registerType"];
    }
    else if (registerType == 1)
    {
        [parameters setObject:@(1) forKey:@"registerType"];
    }
    /*
