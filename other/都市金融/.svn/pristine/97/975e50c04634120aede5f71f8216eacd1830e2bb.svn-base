...
static CGFloat const WMProgressHeight = 0.5;
...
- (void)addItems{
    ...
    for (int i = 0; i < self.items.count; i++) {
        ...
        if (i != (self.items.count-1)) {
            UIView *lineV = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetWidth(item.frame)-lineW, (CGRectGetHeight(frame)-CGRectGetHeight(frame)/5)/2, lineW, CGRectGetHeight(frame)/5)];
...
- (void)didPressedMenuItem:(WKMenuItem *)menuItem{
    ...
    if ([self.delegate respondsToSelector:@selector(menuView:didSelesctedIndex: moveEnd:)]) {
//        [self.delegate menuView:self didSelesctedIndex:menuItem.tag-kTagGap moveEnd:YES];
        [self.delegate menuView:self didSelesctedIndex:[self.items indexOfObject:menuItem.title] moveEnd:YES];
    }
