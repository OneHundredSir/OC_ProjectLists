...
- (void)initView
{
    ...
    [_ComboBox setArrayData:dataArr];
     _ComboBox.table.frame= CGRectMake(_ComboBox.frame.origin.x, _ComboBox.frame.origin.y+86, _ComboBox.frame.size.width, [dataArr count]*30);
    ...
    henlabel.text = @"---";
    henlabel.textAlignment = NSTextAlignmentCenter;
    ...
    // 时间选择器背景View
    _timeView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height, self.view.frame.size.width, 260)];
...
- (void)ViewAnimation:(UIView*)view willHidden:(BOOL)hidden {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        if (hidden) {
            _backView.alpha = 0;
            _listView.scrollEnabled = YES;
            view.frame = CGRectMake(0, self.view.frame.size.height, self.view.frame.size.width, 260);
        } else {
            _backView.alpha = .5;
            _listView.scrollEnabled = NO;
            [view setHidden:hidden];
            view.frame = CGRectMake(0, self.view.frame.size.height - 220, self.view.frame.size.width, 260);
        }
...
- (void)requestData {
    if (AppDelegateInstance.userInfo == nil) {
        ...
        [parameters setObject:[NSString stringWithFormat:@"%ld", (long)_keyType] forKey:@"type"];
