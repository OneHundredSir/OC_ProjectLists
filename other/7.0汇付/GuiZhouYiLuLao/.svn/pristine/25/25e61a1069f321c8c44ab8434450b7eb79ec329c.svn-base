...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if ([currentURL rangeOfString:@"appCallBack"].location != NSNotFound || [currentURL rangeOfString:@"/front/PaymentAction"].location != NSNotFound || [currentURL rangeOfString:@"/payment/payErrorInfo"].location != NSNotFound|| [currentURL rangeOfString:@"/payment"].location != NSNotFound) {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
   //1.开户 2.充值 3.投标 4.提现 5.绑定银行卡 6.债权转让竞拍确认（竟价）7.债权转让竞拍确认受让（定向）8.还款
    ...
    else if([self.type isEqualToString:@"5"])//绑卡
    {
        [self.navigationController popViewControllerAnimated:YES];
    }
    
    else if ([self.type isEqualToString:@"6"]||[self.type isEqualToString:@"7"])
    {// 接受转让（竟价）6（定向）7
    ...
    else if ([self.type isEqualToString:@"8"])//还款
    {
