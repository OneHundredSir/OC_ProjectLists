...
+ (void)showSuccessWithStatus:(NSString *)string {
    ...
    [self showImage:SVProgressHUDSuccessImage status:string isAuto:YES];
}

+ (void)showErrorWithStatus:(NSString *)string {
    [self sharedView];
    [self setErrorImage:[UIImage imageNamed:@"hud_error"]];
    [self showImage:SVProgressHUDErrorImage status:string isAuto:YES];
}
+ (void)showSuccessWithStatusNoAuto:(NSString*)string;
{
    [self sharedView];
    [self setSuccessImage:[UIImage imageNamed:@"hud_success"]];
    [self showImage:SVProgressHUDSuccessImage status:string isAuto:NO];
    
}
+ (void)showErrorWithStatusNoAuto:(NSString *)string
{
    [self sharedView];
    [self setErrorImage:[UIImage imageNamed:@"hud_error"]];
    [self showImage:SVProgressHUDErrorImage status:string isAuto:NO];
    
}

+ (void)showImage:(UIImage*)image status:(NSString*)string isAuto:(BOOL)isAuto
{
    NSTimeInterval displayInterval = [[SVProgressHUD sharedView] displayDurationForString:string];
    [[self sharedView] showImage:image status:string duration:displayInterval isAuto:isAuto];
}
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration  isAuto:(BOOL)isAuto{
    self.progress = -1;
    [self cancelRingLayerAnimation];
