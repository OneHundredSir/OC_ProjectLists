...
@interface AuctionViewController ()<UITextFieldDelegate,HTTPClientDelegate,UIScrollViewDelegate,UIAlertViewDelegate>
{
...
- (void)initView1
{
    ...
    //滚动视图
    _scrollView =[[MSKeyboardScrollView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, MSHIGHT+64)];
...
//    
    ...
    _tenderBtn.backgroundColor = BrownColor;
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (textField.tag == 101) {
        ...
        if ([_TendermoneyField.text floatValue] < minAmount) {
...
//            _rentalLabel.text = [NSString stringWithFormat:@"¥%0.1f",[[dics objectForKey:@"accountAmount"] floatValue]];
            _balancelLabel.text = [NSString stringWithFormat:@"%0.2f",[[dics objectForKey:@"availableBalance"] floatValue]];
            ...
            _TendermoneyField.text = [NSString stringWithFormat:@"%ld",(long)[_minValue.text integerValue]];
...
//        NSString *imagePath = [[NSBundle mainBundle] pathForResource:@"logo" ofType:@"png"];
        ...
        //构造分享内容
        id<ISSContent> publishContent = [ShareSDK content:@"一路捞 我要投资 借款详情"
