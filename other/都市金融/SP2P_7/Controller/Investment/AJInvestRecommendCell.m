...
@interface AJInvestRecommendCell ()
...
@end
@implementation AJInvestRecommendCell
...
{
    static NSString *ID = @"AJInvestRecommendCell";
    AJInvestRecommendCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[AJInvestRecommendCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
...
- (void)setAInvestment:(Investment *)aInvestment
{
    ...
    self.progress.text = [NSString stringWithFormat:@"进度 %d%@",(int)(aInvestment.progress + 0.5),@"%"];
    self.amount.text = [NSString stringWithFormat:@"金额 %.2f",aInvestment.amount];
