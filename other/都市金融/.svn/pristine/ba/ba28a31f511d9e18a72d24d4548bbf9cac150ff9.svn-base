...
@property (nonatomic,strong)UILabel *titleLabel; //借款标题
...
- (void)initView
{
    ...
    //滚动视图
    _ScrollView =[[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.bounds.size.height - 64)];
    ...
    // 1.编号
    CGFloat _idLabelX = 15.f;
    CGFloat _idLabelY = 10.f;
    CGFloat _idLabelW = 60.f;
    _idLabel = [[UILabel alloc] initWithFrame:CGRectMake(_idLabelX, _idLabelY, _idLabelW, 30)];
    ...
    // 借款标题
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_idLabel.frame) + 5, _idLabelY, MSWIDTH - 120, 30)];
...
//    _collectBtn = [UIButton buttonWithType:UIButtonTypeCustom]; // 20160122--取消收藏
    ...
    _roundimgView = [[UIImageView alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 40, 35, 30, 30)];
    _roundimgView.image = [UIImage imageNamed:@"round_back"];
    [_ScrollView addSubview:_roundimgView];
    
    _progressLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 37, 42, 30, 15)];
    ...
    for (int i = 0; i<[titleArr count]; i++){
        ...
        if (i==3||i==4) {
            ...
            textLabel.frame = CGRectMake(155, 70+20*(i-3), 60, 20);
            
        }else {
            textLabel.frame = CGRectMake(10, 70+20*i, 60, 20);
        }
    ...
    _deadlineLabel = [[UILabel alloc] initWithFrame:CGRectMake(220, 70, 100, 20)];
    ...
    _dateLabel = [[UILabel alloc] initWithFrame:CGRectMake(220, 90, 80, 20)];
    _dateLabel.textColor = [UIColor darkGrayColor];
    _dateLabel.font = [UIFont systemFontOfSize:12.0f];
    [_ScrollView addSubview:_dateLabel];
    
    CGFloat _dateBackViewY = CGRectGetMaxY(_wayLabel.frame) + 5;
    _dateBackView = [[UIView alloc] initWithFrame:CGRectMake(10, _dateBackViewY, 110, 30)];
    _dateBackView.backgroundColor = SETCOLOR(229, 59, 80, 1);
    [_ScrollView addSubview:_dateBackView];
    
    _dateLabel1 = [[UILabel alloc] initWithFrame:CGRectMake(50, _dateBackViewY, 110, 30)];
    ...
    _clockImg = [[UIImageView alloc] initWithFrame:CGRectMake(20, _dateBackViewY + 5, 22, 22)];
    _clockImg.image = [UIImage imageNamed:@"clock_big"];
    _clockImg.backgroundColor = [UIColor clearColor];
    [_ScrollView addSubview:_clockImg];
    
    _dateLabel2 = [[UILabel alloc] initWithFrame:CGRectMake(120, _dateBackViewY, self.view.frame.size.width - 130, 30)];
    ...
    _listView = [[UIFolderTableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_dateBackView.frame) + 5, self.view.frame.size.width, self.view.bounds.size.height - 195 - 49)  style:UITableViewStyleGrouped];
...
//    _listView.backgroundColor = [UIColor redColor];
    ...
    //底部背景视图
    CGFloat _bottomViewH = 64;
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 64 - _bottomViewH, MSWIDTH, _bottomViewH)];
...
//            DLOG(@"%@", self.view.subviews);
            ...
            tenderBtn.frame = CGRectMake(padding, 5, MSWIDTH - 2*padding,49);
            ...
            tenderBtn.layer.cornerRadius = 6;
            tenderBtn.layer.masksToBounds = YES;
...
//                _ScrollView.contentSize = CGSizeMake(self.view.frame.size.width, 1080);
    ...
    _organizationtextLabel.font = _wayLabel.font;
    _organizationtextLabel.textAlignment = NSTextAlignmentRight;
    _organizationtextLabel.frame = CGRectMake(_idLabelX, CGRectGetMaxY(_wayLabel.frame), 60, 20);
    [_ScrollView addSubview:_organizationtextLabel];
    _organizationtextLabel.hidden = YES;
    
    _organizationLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_organizationtextLabel.frame), CGRectGetMaxY(_wayLabel.frame), 150, 20)];
    ...
    _organizationLabel.hidden = YES;
    
    [self.listView reloadData];
    dispatch_async(dispatch_get_main_queue(), ^{
        self.ScrollView.contentSize = CGSizeMake(0, self.listView.frame.origin.y + self.listView.contentSize.height);
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        if (num2 == 1) {
            ...
            _idLabel.text = [NSString stringWithFormat:@"编号：%@",dics[@"no"]];
...
//                    [_bottomView removeFromSuperview];
            ...
            {
                _organizationLabel.hidden = _organizationtextLabel.hidden = NO;
                
                CGRect listViewF = self.listView.frame;
                listViewF.origin.y += _organizationLabel.bounds.size.height + 5;
                self.listView.frame = listViewF;
                
                CGRect dateBackViewF = self.dateBackView.frame;
                dateBackViewF.origin.y += _organizationLabel.bounds.size.height + 5;
                self.dateBackView.frame = dateBackViewF;

                CGRect dateLabel1F = self.dateLabel1.frame;
                dateLabel1F.origin.y += _organizationLabel.bounds.size.height + 5;
                self.dateLabel1.frame = dateLabel1F;
                
                CGRect dateLabel2F = self.dateLabel2.frame;
                dateLabel2F.origin.y += _organizationLabel.bounds.size.height + 5;
                self.dateLabel2.frame = dateLabel2F;
                
                CGRect clockImgF = self.clockImg.frame;
                clockImgF.origin.y += _organizationLabel.bounds.size.height + 5;
                self.clockImg.frame = clockImgF;
...
//                _listView.frame = CGRectMake(0, 150, self.view.frame.size.width, 800);
            ...
            if ([_progressLabel.text isEqualToString:@"100%"])
            {
                ...
                CGRect listViewF = self.listView.frame;
               listViewF.origin.y -= _dateBackView.bounds.size.height + 5;
                self.listView.frame = listViewF;
                
//                _listView.frame = CGRectMake(0, 150, self.view.frame.size.width, );
            }
            ...
            //借贷记录
            NSString *borrowStr = [NSString stringWithFormat:@"贷款记录: %@ 次成功, %@ 次失败",_BorrowsucceedStr,_BorrowfailStr];
            ...
            [_borrowRecordStr addAttribute:NSForegroundColorAttributeName value:kAttentionColor range:NSMakeRange(6,nameLen)];
            [_borrowRecordStr addAttribute:NSForegroundColorAttributeName value:kAttentionColor range:NSMakeRange(nameLen+12,amountLen)];
            
            //还款记录
            NSString *repayStr = [NSString stringWithFormat:@"还款记录: %@ 次正常, %@ 次逾期",_repaymentnormalStr,_repaymentabnormalStr];
            ...
            [_repayRecordStr addAttribute:NSForegroundColorAttributeName value:kAttentionColor range:NSMakeRange(6,fristLen)];
            [_repayRecordStr addAttribute:NSForegroundColorAttributeName value:kAttentionColor range:NSMakeRange(fristLen+12,secondLen)];
            ...
            [_listView reloadData];
            dispatch_async(dispatch_get_main_queue(), ^{
                //刷新完成时更新scrollView的contentSize
                self.ScrollView.contentSize = CGSizeMake(0, CGRectGetMinY(self.listView.frame)+ self.listView.contentSize.height + 30);
...
- (void)tableViewAddCutHightWithExpandController:(UIViewController*)controller isAdd:(BOOL)isAdd
{
    if (isAdd) {
        CGFloat contentH = self.listView.contentSize.height + self.listView.frame.origin.y + controller.view.bounds.size.height;
        CGFloat ScrollViewcontentH = (contentH > self.view.frame.size.height)?contentH:self.view.bounds.size.height;
        self.ScrollView.contentSize = CGSizeMake(0, ScrollViewcontentH);
        
        CGRect frame = self.listView.frame;
        frame.size.height += controller.view.frame.size.height;
        self.listView.frame = frame;
        ...
        frame.size.height -= controller.view.frame.size.height;
        self.listView.frame = frame;
        CGFloat contentH = self.ScrollView.contentSize.height -  controller.view.frame.size.height;
        CGFloat ScrollViewcontentH = (contentH > self.view.frame.size.height)?contentH:self.view.bounds.size.height;
        self.ScrollView.contentSize = CGSizeMake(0,ScrollViewcontentH);
        ...
        case 1:
        {
            ...
            [folderTableView openFolderAtIndexPath:indexPath WithContentView:DetailsDescriptionView.view
                                         openBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                             ...
                                             [weakSelf tableViewAddCutHightWithExpandController:DetailsDescriptionView isAdd:YES];
                                        ...
                                        closeBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                            ...
                                            [weakSelf tableViewAddCutHightWithExpandController:DetailsDescriptionView isAdd:NO];
                                        }
                                   completionBlock:nil];
        ...
        case 2:
        {
            ...
            [folderTableView openFolderAtIndexPath:indexPath WithContentView:MaterialAuditSubjectView.view
                                         openBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                            ...
                                            [weakSelf tableViewAddCutHightWithExpandController:MaterialAuditSubjectView isAdd:YES];                                         }
                                        closeBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                            
                                            [weakSelf tableViewAddCutHightWithExpandController:MaterialAuditSubjectView isAdd:NO];
                                        }
                                   completionBlock:nil];
        ...
        case 3:
        {
            ...
            [folderTableView openFolderAtIndexPath:indexPath WithContentView:CBORiskControlSystemView.view
                                         openBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                             ...
                                             btn.selected = !btn.selected;
//
                                              [weakSelf tableViewAddCutHightWithExpandController:CBORiskControlSystemView isAdd:YES];
                                         }
                                        closeBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                            [weakSelf tableViewAddCutHightWithExpandController:CBORiskControlSystemView isAdd:NO];
                                        }
                                   completionBlock:nil];
        ...
        case 4:
        {
            ...
            [folderTableView openFolderAtIndexPath:indexPath WithContentView:HistoricalRecordView.view
                                         openBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                             ...
                                             //[self CloseAndOpenACtion:indexPath];
                                             btn.selected = !btn.selected;
                                             [weakSelf tableViewAddCutHightWithExpandController:HistoricalRecordView isAdd:YES];
                                         }
                                        closeBlock:^(UIView *subClassView, CFTimeInterval duration, CAMediaTimingFunction *timingFunction){
                                            // closing actions
                                            [weakSelf tableViewAddCutHightWithExpandController:HistoricalRecordView isAdd:NO];
                                        }
                                   completionBlock:nil];
...
- (void)tenderBtnClick
{
    ...
    if (AppDelegateInstance.userInfo == nil) {
        ...
        //        tenderOnceView.apr = apr;
        //        tenderOnceView.period = deadLine;
        //        tenderOnceView.rType = repayType;
        //        tenderOnceView.unitType = deadperiodUnit;
...
- (void)postInfoClick
{
    ...
    literatureAuditView.backType = 1;
    ...
    [self.requestClient requestParameters:parameters success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject[@"error"] intValue] == -1) {
            ...
            if ([[responseObject allKeys] containsObject:@"businessLicense"]) { //企业
                ...
                AJBorrowerInfoCoontroller *BorrowerInformationView = [[AJBorrowerInfoCoontroller alloc] init];
                [BorrowerInformationView setValue:@NO forKey:@"ipsAcctNo"];
                [BorrowerInformationView setValue:@"借款人信息" forKey:@"title"];
                [BorrowerInformationView setValue:responseObject forKey:@"responseObj"];
