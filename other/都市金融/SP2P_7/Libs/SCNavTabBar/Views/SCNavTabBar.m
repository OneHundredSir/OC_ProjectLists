...
- (void)viewConfig
{
    ...
    {
        // 周利强
        CGFloat _arrowButtonW = 0.f;
        _arrowButton = [[UIImageView alloc] initWithFrame:CGRectMake(functionButtonX, DOT_COORDINATE, _arrowButtonW, ARROW_BUTTON_WIDTH)];
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(5.0f, NAV_TAB_BAR_HEIGHT - 2.0f, width , 2.0f)];
//    _line.backgroundColor = UIColorWithRGBA(20.0f, 80.0f, 200.0f, 0.7f);
    _line.backgroundColor = UIColorWithRGBA(239,90,80,1.0);
...
//    CGFloat buttonX = DOT_COORDINATE;
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        [button setTitleColor:[ColorTools colorWithHexString:@"#646464"] forState:UIControlStateNormal];
        [button setTitleColor:[ColorTools colorWithHexString:@"#ef5a50"] forState:UIControlStateSelected];
        if (index == 0) {
            button.selected = YES;
        }
...
#define kMinWidth 40.f
    for (NSString *title in titles)
    {
//        CGSize size = [title sizeWithFont:[UIFont systemFontOfSize:[UIFont systemFontSize]]];
//        NSNumber *width = [NSNumber numberWithFloat:size.width + kMinWidth];
        NSNumber *width = [NSNumber numberWithFloat:([UIScreen mainScreen].bounds.size.width - 10)/ titles.count];
...
//    _isOldBtn = !_isOldBtn;
//    UIColor *color = _isOldBtn?[ColorTools colorWithHexString:@"#646464"]:[ColorTools colorWithHexString:@"#ef5a50"]; // 周利强
    _currentItemIndex = currentItemIndex;
    UIButton *button = _items[currentItemIndex];
    for (UIButton *btn in _items) {//
        btn.selected = NO;//
    }//
//    [_items makeObjectsPerformSelector:@selector(setSelected:) withObject:@NO];
