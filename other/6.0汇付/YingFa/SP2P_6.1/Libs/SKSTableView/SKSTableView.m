...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (![self.expandedIndexPaths[indexPath.section] containsObject:indexPath]) {
        ...
        //cell.backgroundColor = [self separatorColor];
           cell.backgroundColor = KblackgroundColor;
...
- (NSInteger)subRow
{
    id subRowObj = objc_getAssociatedObject([SKSTableView class], SubRowObjectKey);
...
- (void)setSubRow:(NSInteger)subRow
{
    ...
    objc_setAssociatedObject([SKSTableView class], SubRowObjectKey, subRowObj, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
