...
- (void) setArrayDatas:(NSArray *)array
{
    button.hidden=YES;
    [arrayData release];
    arrayData=[array retain];
    dropDownHeight = [arrayData count]*30;
    _table.frame =CGRectMake(_table.frame.origin.x, CGRectGetMaxY(self.frame), _table.frame.size.width, dropDownHeight);
    [_table reloadData];
    [self __show];
}
...
- (void) buttonPressed
{
    ...
    {
        [self.delegate didClickAjComboBox];//点击展开调用方法
    }
}
