...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.isGoodTable =_offsetPoint.x >=_scrollView.frame.size.width?0:1;
...
//        需要网络请求
    ...
    {
        MainPersonViewController *personVC = [[MainPersonViewController alloc]init];
        personVC.view.frame = self.view.frame;
        AuthorModel *model = self.authorArr[indexPath.row];
        #pragma mark 请求到个人页面
        /**
         *  请求到个人数据
         */
        [WHDHttpRequest getUserDetailWithuserId:model.id andandCompletion:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (!error) {
                NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
                
                NSDictionary *resultDic = dic[@"result"];
//                NSLog(@"%@",resultDic);
                personVC.personModel = [PersonModel mj_objectWithKeyValues:resultDic];
                personVC.personModel.myAuth = resultDic[@"newAuth"];//VIP标识符
                [personVC setViewTitle: @"个人中心"];
