...
//    scrollView.keyboardDismissMode = UIScrollViewKeyboardDismissModeOnDrag;
    ...
    CGFloat titlelabelY = 80-64.f;
    for (int i=0; i<3; i++) {
        UILabel *titlelabel = [[UILabel alloc] initWithFrame:CGRectMake(10, titlelabelY+i*70, 60, 30)];
    ...
    CGFloat _nameFieldY = 110-64.f;
    CGFloat recipientsH = 30.f;
    CGFloat _nameFieldH = 35;
    _nameField = [[UITextField alloc] initWithFrame:CGRectMake(15, _nameFieldY, MSWIDTH-30 - recipientsH - 10, _nameFieldH)];
    ...
    _nameField.text = @"";
    [scrollView addSubview:_nameField];
    // 收件人
    UIButton *recipients = [UIButton buttonWithType:UIButtonTypeCustom];
    [recipients setTitle:@"+" forState:UIControlStateNormal];
    [recipients setTitleColor:[ColorTools colorWithHexString:@"#646464"] forState:UIControlStateNormal];
    recipients.backgroundColor = SETCOLOR(230, 230, 230, 1);
    recipients.titleLabel.font = [UIFont boldSystemFontOfSize:25];
    [scrollView addSubview:recipients];
    [recipients addTarget:self action:@selector(selectRecipient:) forControlEvents:UIControlEventTouchUpInside];
    recipients.frame = CGRectMake(MSWIDTH - 15 - recipientsH , _nameFieldY + (_nameFieldH - recipientsH)/2, recipientsH, recipientsH);
    [recipients addTarget:self action:@selector(highlightedChageBgColor:) forControlEvents:UIControlEventTouchDown];
    
    _titleField = [[UITextField alloc] initWithFrame:CGRectMake(15, _nameFieldY + 70, MSWIDTH-30, 35)];
    ...
    _contentView = [[UITextView alloc] initWithFrame:CGRectMake(15, _nameFieldY + 70*2, MSWIDTH-30, 100) ];
 ...
 * 按下按钮改变背景
 */
- (void)highlightedChageBgColor:(UIButton*)sender
{
    sender.backgroundColor = [ColorTools colorWithHexString:@"#c8c8c8"];
}

/**
 * 选择接收人
 */
- (void)selectRecipient:(UIButton*)sender
{
     sender.backgroundColor = SETCOLOR(230, 230, 230, 1);
    
    AJChooseRecipientsController *controller = [[AJChooseRecipientsController alloc] init];
...
{
    CGPoint ContentOffset = CGPointMake(0, textView.frame.origin.y - 10);
