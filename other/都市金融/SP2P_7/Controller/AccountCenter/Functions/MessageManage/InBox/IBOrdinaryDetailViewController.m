...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (statusOPT == 137) {
            ...
            senderName = obj[@"page"][@"sender_name"];
            _inBoxNameLabel.text = [NSString stringWithFormat:@"发件人: %@", obj[@"page"][@"sender_name"]];
            _titleLabel.text = obj[@"page"][@"title"];
            ...
            if (!up && !down) {//无剩余条数，返回列表
                [self.navigationController popViewControllerAnimated:NO];
                return;
            }
            else if (_index == 1 && !up && down) {
                //当剩余总条数为2，并且选择第一条进入详情，则操作状态改为0（当前条数）
                status = 0;
            }else{
                status = 2;
            }
