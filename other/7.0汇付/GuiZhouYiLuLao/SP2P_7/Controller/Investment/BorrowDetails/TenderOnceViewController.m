...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if (_num == 1)
    {
        if ([error isEqualToString:@"-1"])
        {
            ...
            if ([[dics objectForKey:@"minTenderedSum"] intValue] > 0) {
                ...
                [_slider setValue:[_minValue.text floatValue] animated:YES];
    ...
    else if (_num == 2)// 投标
    {
        ...
        else if ([error isEqualToString:@"-31"])
        {
            ...
            openAccountVC.level = self.level+1;
            [self.navigationController pushViewController:openAccountVC animated:YES];
            
        }
        else if ([error isEqualToString:@"-3"])//未完善个人资料
        {
            _tenderBtn.enabled = YES;
            [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"%@", [obj objectForKey:@"msg"]]];
