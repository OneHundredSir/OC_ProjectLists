...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.depictLabel.frame = CGRectMake(cell.depictLabel.frame.origin.x, cell.depictLabel.frame.origin.y, cell.depictLabel.frame.size.width, bean.labHeight);
    
    if (cell.depictLabel.text.length < 16)
    {
        cell.depictLabel.textAlignment = NSTextAlignmentRight;
    }else{
        cell.depictLabel.textAlignment = NSTextAlignmentLeft;
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        for (NSDictionary *item in dataArr) {
            ...
            CGSize labelSiZe = [bean.descriptionStr boundingRectWithSize:CGSizeMake(MSWIDTH-110, 2000)options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
            bean.labHeight = labelSiZe.height;
