...
//    NSLog(@"%@",NSStringFromCGRect(self.view.frame));
    ...
    for (NSDictionary *dicResult in arr)
    {
        ...
        if (![self.nameArr containsObject:name]) {
            [self.nameArr addObject:name];
        }
        
        NSString *idName = dicResult[@"id"];
        if (![self.idArr containsObject:name]) {
            [self.idArr addObject:idName];
        }
        
        
    }
    
    [menuView _setScrollViewWithTitleArray:self.nameArr];
    menuView.btnBlock = ^(UIButton *btn)
    {
        NSInteger index =  btn.tag-10;
        IdDetailViewController *vc = [IdDetailViewController new];
        vc.view.frame = self.view.frame;
        [vc setViewTitle:self.nameArr[index]];
        NSMutableDictionary *paramas = [@{} mutableCopy];
        [paramas setObject:self.idArr[index] forKey:@"action"];
