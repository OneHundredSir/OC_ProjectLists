...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        _repayAmount.text = [NSString stringWithFormat: @"本期应收金额: %.2f元", [[obj objectForKey:@"repayAmount"]floatValue]];
        
        if (![[obj objectForKey:@"repayCapital"] isEqual:[NSNull null]]){
            _repayCapital.text = [NSString stringWithFormat: @"投标本金: %.2f元", [[obj objectForKey:@"repayCapital"]floatValue]];
        }
        ...
        if (![[obj objectForKey:@"expiryDate"] isEqual:[NSNull null]]){
            ...
            _expiryDate.text = [NSString stringWithFormat: @"%@",[obj objectForKey:@"expiryDate"]];
            
            if (_expiryDate.text.length > 19) {
                _expiryDate.text = [_expiryDate.text substringWithRange:NSMakeRange(0, 19)];
            }
            _expiryDate.text = [NSString stringWithFormat:@"还款到期时间: %@",  _expiryDate.text];
        }
        if (![[obj objectForKey:@"repayCapital"] isEqual:[NSNull null]]){
            _repayCapital.text = [NSString stringWithFormat: @"投标本金: %.2f元", [[obj objectForKey:@"repayCapital"]floatValue]];
        }
        ...
        _interestSum.text = [NSString stringWithFormat: @"本息合计应收金额: %.2f元", [[obj objectForKey:@"interestSum"]floatValue]];
        _receivedAmount.text = [NSString stringWithFormat: @"已收金额: %.2f元", [[obj objectForKey:@"receivedAmount"]floatValue]];
        _receivedNum.text = [NSString stringWithFormat: @"已收期数: %@期", [obj objectForKey:@"receivedNum"]];
        _remainNum.text = [NSString stringWithFormat: @"剩余期数: %@期", [obj objectForKey:@"remainNum"]];
        _remainAmount.text = [NSString stringWithFormat: @"剩余应收款: %.2f元", [[obj objectForKey:@"remainAmount"]floatValue]];
