...
- (void)initView
{
    ...
    _leftBtn.tag = 1;
    [_leftBtn setTitle:[NSString stringWithFormat:@"%ld月",monthnum - 1] forState:UIControlStateNormal];
    ...
    _rightBtn.tag = 2;
    [_rightBtn setTitle:[NSString stringWithFormat:@"%ld月",monthnum + 1] forState:UIControlStateNormal];
    ...
    _yearlabel.text = [NSString stringWithFormat:@"%ld年%ld月", (long)year, (long)monthnum];
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        for (NSDictionary *item in collections) {
            ...
            //服务器为6.0时
            bean.avgDeadline = [item objectForKey:@"average_invest_month"]; // 均借款期限(月)
            //服务器为7.0时
//            bean.avgDeadline = [item objectForKey:@"average_inves_period"];
            ...
            bean.avgReturnRate = [NSString stringWithFormat:@"%@", [item objectForKey:@"invest_fee_back"]];          // 均投资回报率
        ...
        if ([dics isEqual:[NSNull null]] || dics == nil) {
            
            DLOG(@"返回失败  msg -> 服务器维护中");
