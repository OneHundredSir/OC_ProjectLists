...
- (id)dequeueReusableCellWithIdentifier:(NSString *)identifier{
    ...
    [self.reusePool enumerateObjectsUsingBlock:^(WKPageCell *cell, BOOL *stop) {// reusePool isKandOfClass:NSSet
...
- (void)menuView:(WKMenuView *)menu didSelesctedIndex:(NSInteger)index moveEnd:(BOOL)isEnd{
    ...
    [self menuView:menu didSelesctedIndex:index];
}
...
- (CGFloat)menuView:(WKMenuView *)menu widthForItemAtIndex:(NSInteger)index{
    ...
    if ([self.delegate respondsToSelector:@selector(pageView:widthForMenuItemAtIndex:)]) {
        CGFloat menuH = [self.delegate pageView:self widthForMenuItemAtIndex:index];
        CGFloat tempH = [UIScreen mainScreen].bounds.size.width/_menuItems.count;
        return (menuH > tempH ? menuH:tempH);
    }else{
...
- (WKPageCell*)cellAtIndex:(int)index
{
