...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        _repayAmount.text = [NSString stringWithFormat: @"本期应收金额: %@元", [obj objectForKey:@"repayAmount"]];
        
        if (![[obj objectForKey:@"repayCapital"] isEqual:[NSNull null]]){
            _repayCapital.text = [NSString stringWithFormat: @"投标本金: %@元", [obj objectForKey:@"repayCapital"]];
        }
        ...
        if (![[obj objectForKey:@"expiryDate"] isEqual:[NSNull null]]){
            ...
            NSString *expiryDate = obj[@"expiryDate"];
//            NSRange range = [expiryDate rangeOfString:@"."];
            expiryDate = [expiryDate substringToIndex:expiryDate.length - 2];
            _expiryDate.text = [NSString stringWithFormat: @"还款到期时间: %@", expiryDate];
        }
        if (![[obj objectForKey:@"repayCapital"] isEqual:[NSNull null]]){
            _repayCapital.text = [NSString stringWithFormat: @"投标本金: %@元", [obj objectForKey:@"repayCapital"]];
        }
        double annualRate = [obj[@"annualRate"] doubleValue];
        _annualRate.text = [NSString stringWithFormat: @"年利率: %.1f%@", annualRate, @"%"];
        _interestSum.text = [NSString stringWithFormat: @"本息合计应收金额: %@元", [obj objectForKey:@"interestSum"]];
        _receivedAmount.text = [NSString stringWithFormat: @"已收金额: %@元", [obj objectForKey:@"receivedAmount"]];
        ...
        _remainAmount.text = [NSString stringWithFormat: @"剩余应收款: %@元", [obj objectForKey:@"remainAmount"]];
