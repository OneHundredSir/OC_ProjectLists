...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (![[obj objectForKey:@"collectCapital"] isEqual:[NSNull null]]) {
            _collectCapital.text = [NSString stringWithFormat:@"%@", [obj objectForKey:@"collectCapital"]];
        }else {
            _collectCapital.text = @"0";
        }
        if (![[obj objectForKey:@"receivedAmount"] isEqual:[NSNull null]]) {
            _receivedAmount.text = [NSString stringWithFormat:@"%@", [obj objectForKey:@"receivedAmount"]];
        }else {
            _receivedAmount.text = @"0";
        }

        if (![[obj objectForKey:@"interestSum"] isEqual:[NSNull null]]) {
             _interestSum.text = [NSString stringWithFormat:@"%@", [obj objectForKey:@"interestSum"]];
        }else {
            _interestSum.text = @"0";
        }
        
        if (![[obj objectForKey:@"remain_received_amount"] isEqual:[NSNull null]] && [obj objectForKey:@"remain_received_amount"] != nil) {
            _remainAmount.text = [NSString stringWithFormat:@"%@", [obj objectForKey:@"remain_received_amount"]];
        }else {
            _remainAmount.text = @"0";
        }
