...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (item.status == -1) {// -1失败
        ...
        directionalTransferView.receivedAmount = [NSString stringWithFormat:@"%@", item.receiving_amount];
        ...
        if (item.type == 1) {//定向确认
            ...
            confirmedView.sign = item.sign;    //成交请求传sign
            confirmedView.signId = item.signId;//查看债权受让详情传signId
            confirmedView.bidName = item.bidName;
            confirmedView.receivedAmount = [NSString stringWithFormat:@"%@", item.receiving_amount];
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (![[obj objectForKey:@"list"] isEqual:[NSNull null]]) {
            ...
            for (NSDictionary *item in collections) {
                ...
                bean.receiving_amount = [item objectForKey:@"receiving_amount"];
