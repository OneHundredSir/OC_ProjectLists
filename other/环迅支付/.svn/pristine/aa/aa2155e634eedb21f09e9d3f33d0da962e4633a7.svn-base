...
#import "MRepaymentViewController.h"
...
@interface MRepaymentViewController ()<UIScrollViewDelegate, UITextFieldDelegate, AJComboBoxDelegate, HTTPClientDelegate, UITableViewDelegate, UITableViewDataSource>
{
...
@implementation MRepaymentViewController
...
- (void)initView
{
    ...
    // 月还本息
    UILabel *titlelabel32 = [[UILabel alloc] initWithFrame:CGRectMake(80, 50, 120, 30)];
    ...
    // 本息余额
    UILabel  *titlelabel33 = [[UILabel alloc] initWithFrame:CGRectMake(190, 50, 100, 30)];
...
- (void)calculateBtnClick:(UIButton *)btn
{
    ...
    if ([textfield1.text length]==0||[textfield2.text length]==0||[textfield3.text length]==0)
    {
        ...
        [parameters setObject:[NSString stringWithFormat:@"%@",[productIdArr objectAtIndex:_wayComboBox.selectedIndex]] forKey:@"repayWay"];
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (_typeNum == 1) {
            ...
            if (repayments.count && ![repayments isEqual:[NSNull null]]) {
                ...
                wayArr = [dataArr objectAtIndex:0];
                  [_wayComboBox setArrayData:wayArr];
                _wayComboBox.table.frame = CGRectMake(_wayComboBox.frame.origin.x + 8, _wayComboBox.frame.origin.y + 30 - _scrollView.contentOffset.y, _wayComboBox.frame.size.width, [wayArr count] * 30);
            }
...
- (void)setView {
    ...
    ratelabel.text = [NSString stringWithFormat:@"%0.0f", _repaymentCal.monRate * 100];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    _wayComboBox.table.frame = CGRectMake(_wayComboBox.frame.origin.x + 8, _wayComboBox.frame.origin.y + 30 - scrollView.contentOffset.y, _wayComboBox.frame.size.width, [wayArr count] * 30);
