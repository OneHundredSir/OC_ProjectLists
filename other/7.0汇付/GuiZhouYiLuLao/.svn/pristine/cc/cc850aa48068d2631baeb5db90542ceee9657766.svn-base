...
- (void)initContentView
{
    ...
    [_passWindow textWithleftImage:@"login_lock" rightImage:@"" placeName:@"输入新密码"];
    ...
    UILabel *label1 = [[UILabel alloc] initWithFrame:CGRectMake(20, 120, 260, 30)];
    label1.text = @"1、6-16位字符（字母 数字 符号)";
    label1.font = [UIFont fontWithName:@"Arial" size:13.0];
    label1.textColor = [UIColor darkGrayColor];
    [self.view addSubview:label1];
    
    UILabel *label2 = [[UILabel alloc] initWithFrame:CGRectMake(20, 145, 260, 50)];
    label2.text = @"2、为了您的账户安全，新旧密码必须不同";
...
- (void)signClick
{
    ...
    if (_passWindow.text.length < 6) {
        DLOG(@"密码格式有误，请重新输入！");
        
        [SVProgressHUD showImage:nil status:@"密码过短"];
        return;
    }
        DLOG(@"提交成功");
...
//        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"重置成功" message:@"恭喜你的！" delegate:self cancelButtonTitle:@"关闭" otherButtonTitles:nil, nil];
//        
//        [alertView show];
