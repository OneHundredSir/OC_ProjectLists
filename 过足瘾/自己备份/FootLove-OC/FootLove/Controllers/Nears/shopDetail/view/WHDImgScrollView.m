...
-(void)initwhdSetAdViewWithImgUrlArr:(NSArray *)ImgUrlArr
{
    ...
    for (int i=0; i<ImgUrlArr.count; i++) {
        ...
        [btn setImage:[UIImage imageNamed:ImgUrlArr[i]] forState:UIControlStateNormal];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    {
        _seletedPageNum = (scrollView.contentOffset.x+W_width)/(W_width*1.0) + 0.5;
    }
//    NSLog(@"%lf,%lf",(scrollView.contentOffset.x+W_width),W_width);
    _myLabel.text = [NSString stringWithFormat:@"%ld/%ld",(long)_seletedPageNum,_totolNum];
...
-(void)configLabel
{
    ...
    label.text = [NSString stringWithFormat:@"%f/%ld",_myscrollView.contentOffset.x/CGRectGetWidth(self.frame),(long)_totolNum];
    _myLabel = label;
    //如果只有一张图片就不显示
    if (_totolNum == 1) {
        label.hidden = YES;
