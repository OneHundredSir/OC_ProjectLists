...
//    _scrollView.contentSize = CGSizeMake(self.view.frame.size.width, 510);
    ...
    _nameLabel.text = [NSString stringWithFormat:@"尊敬的用户%@，您好!",_NameStr];
    ...
    _infoLabel.text = @"     尊敬的客户,为确保您应收账款准确，请您仔细阅读下面需理财款单明细栏目中各借款标信息。";
    ...
    _billdateLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, _billIdLabelY+20, self.view.frame.size.width, 30)];
    ...
    [listViewFooter addSubview:_billdateLabel];
    
    _webLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, _billIdLabelY+20*2, self.view.frame.size.width, 30)];
    _webLabel.text = @"";
    _webLabel.font = [UIFont boldSystemFontOfSize:12.0f];
    _webLabel.textColor = [UIColor grayColor];
    [listViewFooter addSubview:_webLabel];
    
    
    _hotLine = [[UILabel alloc] initWithFrame:CGRectMake(10, _billIdLabelY+20*3, self.view.frame.size.width, 30)];
    _hotLine.text = @"";
    _hotLine.font = [UIFont boldSystemFontOfSize:12.0f];
    _hotLine.textColor = [UIColor grayColor];
    [listViewFooter addSubview:_hotLine];
    
     _scrollView.contentSize = CGSizeMake(self.view.frame.size.width,self.view.bounds.size.height + 30);
}
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        _nameLabel.text = [NSString stringWithFormat:@"尊敬的用户 %@ ，您好!",[obj objectForKey:@"userName"]]; // 用户名
        
        // 去除返回时间后面的.0
        NSString *billDate = obj[@"billDate"];
        billDate = [billDate substringToIndex:billDate.length - 2];
        _billdateLabel.text = [NSString stringWithFormat:@"理财账单生成日期：%@", billDate]; // 账单生成日期
        _hotLine.text =[NSString stringWithFormat:@"客服专线:%@", [obj objectForKey:@"hotline"]];  // 客服专线
        
        _introductionLabel.text =[NSString stringWithFormat:@"     感谢您使用%@借贷服务,我平台客服专线:%@ 竭诚为您服务", [obj objectForKey:@"platformName"], [obj objectForKey:@"hotline"]];
