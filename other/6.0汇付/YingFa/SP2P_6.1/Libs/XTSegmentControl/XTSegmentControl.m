...
#define XTSegmentControlItemFont (14)// 字体大小

#define XTSegmentControlHspace (12)// 横向间距

#define XTSegmentControlLineHeight (3)// 底部的横线的高度

#define XTSegmentControlAnimationTime (0.3)// 动画时间
...
- (void)doTap:(UITapGestureRecognizer *)sender
{
    ...
    __weak typeof(self) weakSelf = self;//non-ARC情况下，__block变量的含义是在Block中引入一个新的结构体成员变量指向这个__block变量，那么__block typeof(self) weakSelf = self;就表示Block别再对self对象retain啦，这就打破了循环引用。
...
- (void)addRedLine
{
    if (!_lineView) {
        ...
        //_lineView.backgroundColor = [UIColor colorWithRed:1.0 green:0.6549 blue:0.0 alpha:1.0];
        _lineView.backgroundColor = PinkColor;
