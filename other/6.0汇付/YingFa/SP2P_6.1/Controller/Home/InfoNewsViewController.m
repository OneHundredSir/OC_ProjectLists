...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (![_contentStr isEqual:[NSNull null]]) {
            ...
            NSLog(@"width: %f", self.view.frame.size.width - 20);
       ...
       NSLog(@"返回失败===========%@",[obj objectForKey:@"msg"]);
...
-(void) httpResponseFailure:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didFailWithError:(NSError *)error
{
  ...
  [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"失败：%d", [error code]]];
...
- (void)ShareClick
{
    if (AppDelegateInstance.userInfo == nil) {
        ...
        //构造分享内容
        id<ISSContent> publishContent = [ShareSDK content:@"sp2p6.0 晓风网贷 财富资讯 详情"
                          ...
                          shareOptions: nil
                                result:^(ShareType type, SSResponseState state, id<ISSPlatformShareInfo> statusInfo, id<ICMErrorInfo> error, BOOL end) {
                                    ...
                                    else if (state == SSResponseStateFail)
                                    {
                                        ...
                                        [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"分享失败,错误码:%d,错误描述:%@", [error errorCode], [error errorDescription]]];
                                    }
                                }];
    }
}
