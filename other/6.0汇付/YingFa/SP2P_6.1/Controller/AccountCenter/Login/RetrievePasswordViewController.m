...
- (void)initContentView
{
    ...
    _verificationBtn.frame = CGRectMake(self.view.frame.size.width - 140, 130, 120, 30);
    _verificationBtn.backgroundColor = GreenColor;
    [_verificationBtn setTitle:@"获取验证码" forState:UIControlStateNormal];
    [_verificationBtn setTitleColor:[UIColor blueColor] forState:UIControlStateHighlighted];
    ...
    // 验证码 输入框
    _verificationWindow = [[LoginWindowTextField alloc] initWithFrame:CGRectMake(20, 170, kWidthWin, 35)];
    [_verificationWindow textWithleftImage:@"login_phone" rightImage:nil placeName:@"请输入验证码"];
    _verificationWindow.keyboardType = UIKeyboardTypeNumberPad;
    ...
    signBtn.frame = CGRectMake(20, 240, kWidthWin, 35);
    signBtn.backgroundColor = GreenColor;
    [signBtn setTitle:@"下一步" forState:UIControlStateNormal];
    [signBtn setTitleColor:[UIColor blueColor] forState:UIControlStateHighlighted];
...
- (void)signClick
{
    ...
    if ((_verificationWindow.text == nil)||!_isPhone) {
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if ([_phoneWindow.text isPhone]) {
       ...
       [SVProgressHUD showErrorWithStatus:@"手机格式错误，请重新输入!"];
    }
}
