...
@interface TenderOnceViewController ()<UITextFieldDelegate,UIAlertViewDelegate,HTTPClientDelegate,UIScrollViewDelegate>
{
    ...
    NSString *availableBalance;//可用余额
    NSString *averageInvestAmount;//份，最小认购金额
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if (_num == 1)
    {
        if ([[dics objectForKey:@"error"] isEqualToString:@"-1"])
        {
            ...
            availableBalance = [NSString stringWithFormat:@"%@",[dics objectForKey:@"availableBalance"]];
...
//            }
            ...
            }else
            {
                ...
                averageInvestAmount = [NSString stringWithFormat:@"%@",[dics objectForKey:@"averageInvestAmount"]];
...
- (void)tenderBtnClick
{
    DLOG(@"%@",_TendermoneyField.text);
    DLOG(@"%@",availableBalance);
    
    if ([_xLabel.text isEqualToString:@"元"]) {
        if ([_TendermoneyField.text integerValue] > [availableBalance integerValue]) {
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"可用余额不足，是否现在充值？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
            alertView.tag = 50;
            [alertView show];
    ...
    if ([_xLabel.text isEqualToString:@"份"]) {
        
        if ([_TendermoneyField.text integerValue]*[averageInvestAmount integerValue] > [availableBalance integerValue]) {
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"可用余额不足，是否现在充值？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
            alertView.tag = 50;
            [alertView show];
