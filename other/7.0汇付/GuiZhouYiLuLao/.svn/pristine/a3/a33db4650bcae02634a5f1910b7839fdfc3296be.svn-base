...
@interface ReleaseBorrowInfoViewController ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,AJComboBoxDelegate,UIScrollViewDelegate,UITextFieldDelegate,UITextViewDelegate,HTTPClientDelegate,UITextViewDelegate>
{
...
//    _scrollView =[[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 1060)];
    ...
    titletextView.backgroundColor = BrownColor;
    ...
    //借款用途选择框
    _ComboBox = [[AJComboBox alloc] initWithFrame:CGRectMake(90, 41, 130, 18)];
    [_ComboBox setLabelText:@"选择借款用途"];
    [_ComboBox setTag:201];
    [_ComboBox setDelegate:self];
    [_scrollView addSubview:_ComboBox];
    
    UILabel  *textlabel11 = [[UILabel alloc] initWithFrame:CGRectMake(20, 70, 80,30)];
    ...
    UITextView *textView = [[UITextView alloc] initWithFrame:CGRectMake(20, 93, self.view.frame.size.width-40, 35)];
    ...
    //标题输入框
    _titleField = [[UITextField alloc] initWithFrame:CGRectMake(90, 72, 190,25)];
    ...
    UITextView *textView2 = [[UITextView alloc] initWithFrame:CGRectMake(85, 150, 200, 22)];
    textView2.textColor = [UIColor lightGrayColor];
    textView2.text = @"100~100000(请输入正整数)";
    ...
    UILabel  *textlabel14 = [[UILabel alloc] initWithFrame:CGRectMake(218, 125, 80,30)];
    ...
    UILabel *textlabel13 = [[UILabel alloc] initWithFrame:CGRectMake(20, 177, 80,30)];
    ...
    //年月日单位选择框
    _unitsComboBox = [[AJComboBox alloc] initWithFrame:CGRectMake(90, 183, 90, 18)];
    ...
    //时长选择框
    _deadlineComboBox = [[AJComboBox alloc] initWithFrame:CGRectMake(185, 183, 90, 18)];
...
//        [_deadlineComboBox setArrayData:_limitTimeArr];
    ...
    //还款方式选择框
    _wayComboBox = [[AJComboBox alloc] initWithFrame:CGRectMake(90, 216, 180, 20)];
    ...
    [_averageField addTarget:self action:@selector(valueChange:) forControlEvents:UIControlEventEditingChanged];
    _averageField.backgroundColor = [UIColor whiteColor];
    [_scrollView addSubview:_averageField];
    
    UILabel *textlabel23 = [[UILabel alloc] initWithFrame:CGRectMake(225, 365, 30,30)];
    ...
    _portionLabel = [[UILabel alloc] initWithFrame:CGRectMake(240, 365, 100,30)];
    _portionLabel.lineBreakMode = NSLineBreakByCharWrapping;
    _portionLabel.font = [UIFont systemFontOfSize:12.0f];
    _portionLabel.textColor = [UIColor redColor];
    [_scrollView addSubview:_portionLabel];
    ...
    UILabel *fulllabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 435, 100,30)];
    ...
    //选择框
    _fullScaleComboBox = [[AJComboBox alloc] initWithFrame:CGRectMake(95, 440, 90, 20)];
    ...
    _postBtnBtn.backgroundColor = BrownColor;
    ...
    titletextView2.backgroundColor = BrownColor;
    ...
    [_detailstextView.layer setBorderColor:[[UIColor darkGrayColor] CGColor]];
    ...
    titletextView3.backgroundColor = BrownColor;
...
#pragma mark - 实现UITextViewDelegate的代理
...
{
    _detailstextView.layer.borderColor = [[UIColor blueColor] CGColor];
    return YES;
}

- (BOOL)textViewShouldEndEditing:(UITextView *)textView
{
    _detailstextView.layer.borderColor=[[UIColor darkGrayColor] CGColor];
    return YES;
}
    ...
    if (_num == 1) {
        ...
        if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
            ...
            if (_isRepayment) {
                ...
                _deadlineComboBox.table.frame= CGRectMake(_deadlineComboBox.frame.origin.x, _deadlineComboBox.frame.origin.y + 20 - _scrollView.contentOffset.y, _deadlineComboBox.frame.size.width, _limitTimeArr.count >= 6?6*30:_limitTimeArr.count*30);
            ...
            for (NSDictionary *dic in wayArr) {
                ...
                NSString *isUse = [NSString stringWithFormat:@"%@",[dic objectForKey:@"isUse"]];
            ...
            _wayComboBox.table.frame= CGRectMake(_wayComboBox.frame.origin.x, _wayComboBox.frame.origin.y+82, _wayComboBox.frame.size.width, _wayArr.count>=6?6*30:_wayArr.count*30);
            
            _fullScaleComboBox.table.frame= CGRectMake(_fullScaleComboBox.frame.origin.x, _fullScaleComboBox.frame.origin.y+82, _fullScaleComboBox.frame.size.width, _fullDateArr.count>=6?6*30:_fullDateArr.count*30);
            _ComboBox.table.frame= CGRectMake(_ComboBox.frame.origin.x, _ComboBox.frame.origin.y+82, _ComboBox.frame.size.width, _dataArr.count>=6?6*30:_dataArr.count*30);
            ...
            if (typeNum == 1) {
                if ([[dics objectForKey:@"loanImageFilename"] hasPrefix:@"http"]) {
...
-(void)didChangeComboBoxValue:(AJComboBox *)comboBox selectedIndex:(NSInteger)selectedIndex
{
    ...
    if (comboBox.tag == 201) {
        ...
        _periodUnitStr = [NSString stringWithFormat:@"%ld",selectedIndex - 1];
...
- (void)valueChange:(UITextField *)textF {
    ...
    if (textF.tag == 304 ) {
        ...
        if (_amountNum) {
            float num = [_averageField.text floatValue];
            float avgNum = _amountNum/num;
            float yuNum = fmodf(_amountNum, num);
            if(yuNum == 0){
            
                _portionLabel.text = [NSString localizedStringWithFormat:@"(合计%.0f份)",avgNum];
                
            }else{
                
                _portionLabel.text = @"(份数不能为小数)";
                
            }
            
            DLOG(@"每个份额:%f 余数为:%f",num,avgNum);
        }
    }
    [self getInterest:_peroidNum units:_unitNum apr:_aprNum amount:_amountNum rType:_rtypeNum];
}
...
- (void)SubmitBtnClick
{
    ...
    if([_bonusTypeStr isEqualToString:@"-1"]){
    
        [SVProgressHUD showErrorWithStatus:@"请设置奖励方式"];
        return;
    }
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    _unitsComboBox.table.frame = CGRectMake(_unitsComboBox.frame.origin.x, _unitsComboBox.frame.origin.y+20-scrollView.contentOffset.y, _unitsComboBox.frame.size.width, unitsArr.count>=6?6*30:unitsArr.count*30);
    _wayComboBox.table.frame= CGRectMake(_wayComboBox.frame.origin.x, _wayComboBox.frame.origin.y+20-scrollView.contentOffset.y, _wayComboBox.frame.size.width, _wayArr.count>=6?6*30:_wayArr.count*30);
    
    _ComboBox.table.frame= CGRectMake(_ComboBox.frame.origin.x, _ComboBox.frame.origin.y+20-scrollView.contentOffset.y, _ComboBox.frame.size.width, _dataArr.count>=6?6*30:_dataArr.count*30);
    _deadlineComboBox.table.frame= CGRectMake(_deadlineComboBox.frame.origin.x, _deadlineComboBox.frame.origin.y+20-scrollView.contentOffset.y, _deadlineComboBox.frame.size.width, _limitTimeArr.count>=6?6*30:_limitTimeArr.count*30);
    
    _fullScaleComboBox.table.frame= CGRectMake(_fullScaleComboBox.frame.origin.x, _fullScaleComboBox.frame.origin.y+20-scrollView.contentOffset.y, _fullScaleComboBox.frame.size.width, _fullDateArr.count>=6?6*30:_fullDateArr.count*30);
