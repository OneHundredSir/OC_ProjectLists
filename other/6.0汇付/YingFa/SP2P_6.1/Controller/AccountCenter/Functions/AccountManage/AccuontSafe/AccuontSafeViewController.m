...
- (void)initData
{
    ...
    _dataArr = [[NSMutableArray alloc] initWithArray:@[@"修改登录密码",@"设置安全问题",@"设置安全邮箱",@"设置安全手机"]];
}
...
- (void)setView {
    ...
    if (_isTeleStatus) {
        [_dataArr replaceObjectAtIndex:3 withObject:@"修改安全手机"];
    }else {
        [_dataArr replaceObjectAtIndex:3 withObject:@"设置安全手机"];
    }
