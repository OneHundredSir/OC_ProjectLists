...
- (void)initView
{
    ...
    _scrollView.contentSize = CGSizeMake(self.view.frame.size.width, 690);
    [self.view addSubview:_scrollView];
    
    _topView = [[UIView alloc] initWithFrame:CGRectMake(10, 10, MSWIDTH-20, 320)];
    ...
    _periodUnitLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX([self.view viewWithTag:(102)].frame)+5, 100, 30, 30)];
    ...
    [calculateBtn setFrame:CGRectMake(self.view.frame.size.width*0.5 - 40, 275, 80, 30)];
    ...
    // 标题文本
    UILabel *heardlabel =[[UILabel alloc] initWithFrame:CGRectMake(10, 10, _downView.frame.size.width-20, 40)];
...
- (void)loadRequest {
    ...
    if (self.deadperiodUnit==1) {
        [parameters setObject:@"1" forKey:@"loadType"];//天
    }else{
        [parameters setObject:@"2" forKey:@"loadType"];//月、年
    }
