...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([self.delegate respondsToSelector:@selector(SCNavTabBarController:didSelectedSubController:)]) {
        [self.delegate SCNavTabBarController:self didSelectedSubController:self.subViewControllers.firstObject];
    }
}
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView// 周利强
{
     _currentIndex = scrollView.contentOffset.x / SCREEN_WIDTH;
     _navTabBar.currentItemIndex = _currentIndex;
    
    if ([self.delegate respondsToSelector:@selector(SCNavTabBarController:didSelectedSubController:)]) {
        [self.delegate SCNavTabBarController:self didSelectedSubController:self.subViewControllers[_currentIndex]];
    }
}
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView // 周利强
{
    _currentIndex = scrollView.contentOffset.x / SCREEN_WIDTH;
    _navTabBar.currentItemIndex = _currentIndex;
    if ([self.delegate respondsToSelector:@selector(SCNavTabBarController:didSelectedSubController:)]) {
