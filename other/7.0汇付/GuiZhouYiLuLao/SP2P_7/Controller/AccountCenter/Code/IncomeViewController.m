...
//        _scrollView.delegate = self;
       ...
       // }];
        ...
        DetailBut.backgroundColor = BrownColor;
...
- (void)detailClick
{
    ExtensionBaseViewController *member = [[ExtensionBaseViewController alloc] init];
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (collections.count) {
            ...
            NSArray *dataArr = [collections objectAtIndex:0];
           
            if (dataArr.count)
            {
                Income *bean = [[Income alloc] init];
                
                bean.dateTime = [dataArr objectAtIndex:0];
                int curr = (int)[[dataArr objectAtIndex:1]integerValue];
                bean.incomeMember = [dataArr objectAtIndex:2];
                bean.yseMember = [dataArr objectAtIndex:3];
                bean.noMember = [dataArr objectAtIndex:4];
                bean.cpsBonus = [dataArr objectAtIndex:5];
