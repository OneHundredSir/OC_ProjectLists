//
//  SuccessfulTenderIntegralSubsidiaryViewController.m
//  SP2P_6.1
//
...
@interface SuccessfulTenderIntegralSubsidiaryViewController ()<UITableViewDataSource,UITableViewDelegate, HTTPClientDelegate>
{
...
@implementation SuccessfulTenderIntegralSubsidiaryViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc]init];
      ...
      CGSize LabelSiZe = [cell.otherLabel.text boundingRectWithSize:CGSizeMake(999, 20)options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
    cell.otherLabel.frame =  CGRectMake(140,35, LabelSiZe.width+15, 20);
    
    [cell.backView setImage:[UIImage imageNamed:@"integral_red_back"]];
    
    SuccessfulTenderIntegral *bean = _dataArrays[indexPath.section];
    
    cell.typeRow = 3;
    [cell fillCellWithObject:bean];
    
//    cell.idLabel.text = [NSString stringWithFormat:@"编号：%@", bean.bidNo];
//    cell.scoreLabel.text = [NSString stringWithFormat:@"+%d", bean.score];
//    cell.timeLabel.text = bean.investTime;
//    cell.titleLabel.text = bean.title;
//    cell.otherLabel.text = bean.name;
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        for (NSDictionary *item in dataArr) {
            ...
            SuccessfulTenderIntegral *bean = [[SuccessfulTenderIntegral alloc] init];
            
            bean.title = [item objectForKey:@"title"];// 借款标名称
            if (![[item objectForKey:@"invest_time"] isEqual:[NSNull null]]) {
                NSDate *date = [NSDate dateWithTimeIntervalSince1970: [[[item objectForKey:@"invest_time"] objectForKey:@"time"] doubleValue]/1000];
                NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
                [dateFormat setDateFormat:@"yyyy-MM-dd"];
                bean.investTime  = [dateFormat stringFromDate: date];
            }
            bean.score = [[item objectForKey:@"score"] intValue];// 信用积分
            bean.bidNo = [item objectForKey:@"bid_no"] ; // 借款标编号
            bean.name = [item objectForKey:@"name"] ; // 借款人名称
