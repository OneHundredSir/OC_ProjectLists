...
- (void)initView
{
    ...
    CGFloat titlelabel1Y = 80.f - 64;
    CGFloat navBarH = 64;
    UILabel *titlelabel1 = [[UILabel alloc] initWithFrame:CGRectMake(10, titlelabel1Y, 120, 30)];
    ...
    // 输入框
    _safeEmailtextfield = [[UITextField alloc] initWithFrame:CGRectMake(10, 120 - navBarH, self.view.frame.size.width-20, 45)];
    ...
    _safeEmailtextfield.placeholder = @"请输入安全邮箱";
    _safeEmailtextfield.backgroundColor = [UIColor whiteColor];
    _safeEmailtextfield.returnKeyType = UIReturnKeyDone;
    _safeEmailtextfield.enablesReturnKeyAutomatically = YES;
    [self.view addSubview:_safeEmailtextfield];
    
    
    //保存并激活按钮
    UIButton *ChangeBtn = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    ChangeBtn.frame = CGRectMake(10,CGRectGetMaxY(_safeEmailtextfield.frame) + 30, self.view.frame.size.width-20, 45);
...
//        return;
//    }
    if ([NSString validateEmail:self.safeEmailtextfield.text]) {
        
         [self requestData];
    }else{
        [SVProgressHUD showErrorWithStatus:@"您输入的邮箱格式不正确"];
    }
