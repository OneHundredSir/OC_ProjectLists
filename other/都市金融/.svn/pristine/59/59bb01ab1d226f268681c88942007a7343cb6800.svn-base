...
- (void)initView
{
    ...
    // 1.下拉刷新(进入刷新状态就会调用self的 headerRereshing)
...
-(void)deleteClick
{
    ...
    if (selectedRows.count) {
        ...
        id delegate = nil;
        if (_dataArrays.count) delegate = self;
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"删除" message:@"确定要删除全部信息吗？" delegate:delegate cancelButtonTitle:@"确定" otherButtonTitles:@"取消", nil];
