...
//    _scrollView.backgroundColor = [UIColor blueColor];////
    ...
    UILabel *textLabel2 = [[UILabel alloc] initWithFrame:CGRectMake(10, 140, self.view.frame.size.width-20, 60)];
    ...
    textLabel2.text = @"本期账单还款日";
    [_scrollView addSubview:textLabel2];
    
    _timeLabel = [[UILabel alloc]  initWithFrame:CGRectMake(10, 170, self.view.frame.size.width - 2*10, 50)];
    _timeLabel.font = [UIFont fontWithName:@"Arial-BoldMT" size:16.0];
    _timeLabel.textColor = PinkColor;
     _timeLabel.textAlignment = NSTextAlignmentCenter;
    [_scrollView addSubview:_timeLabel];
    

    UILabel *textLabel3 = [[UILabel alloc] initWithFrame:CGRectMake(10, 205+5, self.view.frame.size.width-20, 60)];
...
//    CGFloat
    _listView = [[UIFolderTableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(textLabel3.frame)+5, self.view.frame.size.width, 400-30)  style:UITableViewStyleGrouped];
...
//     CGFloat listViewFooterY = 120;
    UIView *listViewFooter = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0, 240)];
//    listViewFooter.backgroundColor = [UIColor redColor];
    _listView.tableFooterView = listViewFooter;
   
     CGFloat _billIdLabelY = 10;
    _billIdLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, _billIdLabelY, self.view.frame.size.width, 30)];
    _billIdLabel.font = [UIFont boldSystemFontOfSize:12.0f];
    _billIdLabel.textColor = [UIColor grayColor];
    [listViewFooter addSubview:_billIdLabel];
    
    _billIdDateLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, _billIdLabelY +20, self.view.frame.size.width, 30)];
    _billIdDateLabel.font = [UIFont boldSystemFontOfSize:12.0f];
    _billIdDateLabel.textColor = [UIColor grayColor];
    [listViewFooter addSubview:_billIdDateLabel];
    
    _platformLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, _billIdLabelY + 20*2, self.view.frame.size.width, 30)];
    _platformLabel.font = [UIFont boldSystemFontOfSize:12.0f];
    _platformLabel.textColor = [UIColor grayColor];
    [listViewFooter addSubview:_platformLabel];
    
    
    _hotlineLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, _billIdLabelY+20*3, self.view.frame.size.width, 30)];
    _hotlineLabel.font = [UIFont boldSystemFontOfSize:12.0f];
    _hotlineLabel.textColor = [UIColor grayColor];
    [listViewFooter addSubview:_hotlineLabel];
    
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 90-30, self.view.frame.size.width, 90)];
    ...
    {
        hasBtnH = 30.f;
        bottomView.frame = CGRectMake(0, self.view.frame.size.height - 90-30 - 30, self.view.frame.size.width, 90);
    ...
    _scrollView.contentSize = CGSizeMake(self.view.frame.size.width, 510 + 60 + hasBtnH);
}
...
-(void) setView
{
    ...
    NSDate *billDetailDate = [fmt dateFromString:billDetail.produceBillTime];
    fmt.dateFormat = @"HH:mm";
    NSString *repayHourMinute = [fmt stringFromDate:billDetailDate];
    NSString *timeLabel = [NSString stringWithFormat:@"%@ %@",billDetail.repaymentTime, repayHourMinute];
    _timeLabel.text = timeLabel;

    _nameLabel.text = [NSString stringWithFormat:@"尊敬的用户 %@ 您好!", billDetail.userName];
    ...
    // 账户总额
    _amountLabel.text = [NSString stringWithFormat:@"%0.2f 元", [billDetail.userAmount doubleValue]];
    // 可用余额
    _balanceLabel.text =  [NSString stringWithFormat:@"%0.2f 元", [billDetail.userBalance doubleValue]];
}
...
//    [_requestClient requestGet:self withParameters:parameters payType:TYPE_REPAYMENT navLevel:0];
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if (_requestType == 0)
    {
        if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"])
        {
            ...
            {
                billDetail.produceBillTime = [NSString converDate:[[billDetailDics objectForKey:@"produce_bill_time"] objectForKey:@"time"] withFormat:@"yyyy-MM-dd hh:mm:ss"];
            }
            }
    ...
    else if(_requestType == 1)
    {
        if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
            ...
            [ReLogin outTheTimeRelogin:self];
        }else if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-3"]) {
            //msg  -> 批量还款同步回调 :处理成功
            // 通知全局广播 LeftMenuController 修改UI操作
            [SVProgressHUD dismiss];
