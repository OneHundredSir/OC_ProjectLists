...
- (void)initView
{
    ...
    //_typeImg.image = [UIImage imageNamed:@"small_loan_type1"];
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        if (num == 1) {
            ...
            [_repayRecordStr addAttribute:NSForegroundColorAttributeName value:[UIColor darkGrayColor] range:NSMakeRange(0, borrowStr.length)];
...
//        NSString *imagePath = [[NSBundle mainBundle] pathForResource:@"logo" ofType:@"png"];
        ...
        //构造分享内容
        id<ISSContent> publishContent = [ShareSDK content:@"sp2p 7.1.2善建网贷 理财子账户 投标记录 借款详情"
