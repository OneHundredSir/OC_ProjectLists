...
@interface InvestmentViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,UISearchDisplayDelegate,HTTPClientDelegate,UITextFieldDelegate>
{
...
//    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, _sortContentView.frame.origin.y + _sortContentView.frame.size.height, self.view.frame.size.width, self.view.frame.size.height - _sortContentView.frame.size.height - 64) style:UITableViewStyleGrouped];
     _tableView = [[UITableView alloc] initWithFrame:CGRectMake(5, 64, self.view.frame.size.width-10, self.view.frame.size.height- 113) style:UITableViewStyleGrouped];
...
- (void)initView
{
    ...
    //搜索
    _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 50, MSWIDTH - 120, 40)];
            ...
            for (NSDictionary *item in dataArr) {
                ...
                if ([item objectForKey:@"small_image_filename"]  != nil && ![[item objectForKey:@"small_image_filename"]  isEqual:[NSNull null]]) {
                    ...
                    bean.levelStr = [NSString stringWithFormat:@"%@",[item objectForKey:@"small_image_filename"]];
                      ...
                      bean.levelStr = [NSString stringWithFormat:@"%@%@",Baseurl,[item objectForKey:@"small_image_filename"]];
                ...
                bean.repayTypeStr = [item objectForKey:@"repay_name"];
...
//        {
//             NSLog(@"搜索栏数组%@",subView.subviews);
//            for (UIView* v in subView.subviews)
//            {
//            
//            if ([v isKindOfClass:NSClassFromString(@"UISearchBarTextField")] || [v isKindOfClass:[UITextField class]]) {
//            
...
- (void)ScreenClick
{
    ...
    if (_screenArr.count) {
        controller.oldScreenArr = _screenArr;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.calculatorView.imageView.image = [UIImage imageNamed:@"hhh"];
...
{
    Investment *object = _dataArrays[btn.tag];
