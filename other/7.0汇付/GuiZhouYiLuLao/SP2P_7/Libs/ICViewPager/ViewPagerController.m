...
- (void)drawRect:(CGRect)rect {
    ...
    [bezierPath setLineWidth:0.9];
    [bezierPath stroke];
   // DLOG(@"UIBezierPath%.f",CGRectGetWidth(rect));
    // Draw bottom line
     bezierPath = [UIBezierPath bezierPath];
    ...
    CGFloat lineWidth = MSWIDTH < 375 ? 10.0:15.0;
    ...
    if (self.selected) {
        ...
        bezierPath = [UIBezierPath bezierPath];
        // Draw the indicator
        [bezierPath moveToPoint:CGPointMake(lineWidth, CGRectGetHeight(rect) - 0.9)];
        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect)-lineWidth, CGRectGetHeight(rect) - 0.9)];
        [bezierPath setLineWidth:0.9];
...
- (void)defaultSetup {
   ...
   // NSUInteger index = [self.startFromSecondTab boolValue] ? 1 : 0;
    //    [self selectTabAtIndex:0 didSwipe:NO];
